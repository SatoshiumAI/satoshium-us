...
def object_local_results(results, kind, d):
    """Resolve mandatory rules whose truth is determinable from the serialized object itself."""
    byid={}
    for i,x in enumerate(results): byid.setdefault(x["rule_id"],[]).append(i)

    def resolve(rid, ok, passmsg, failmsg, loc="$"):
        for i in byid.get(rid,[]):
            x=results[i]
            if x["result"]=="not-tested" and x.get("mandatory",True):
                x.update(result="pass" if ok else "fail", location=loc,
                         message=passmsg if ok else failmsg)

    # Representation/profile rules.
    resolve("VAL-REP-001", isinstance(d,dict) and bool(d),
            "Target is a parseable non-empty mapping/object.",
            "Target representation is not a parseable non-empty mapping/object.")
    recognized = ("attestation_identifier" in d) if kind=="ATT" else ("trust_statement_identifier" in d)
    resolve("VAL-REP-002", recognized,
            f"Target is recognized as canonical {kind} base-profile object.",
            "Target object profile is not recognized.")
    legacy_fields={"trust_signal_identifier","trust_signal","signal_strength","trust_score","confidence_score","reputation"}
    resolve("VAL-REP-003", not any(k in d for k in legacy_fields),
            "No legacy Trust Signal representation is used as canonical object vocabulary.",
            "Legacy Trust Signal vocabulary is present in canonical object representation.")

    # Source identifier preservation: references preserve their supplied identifiers.
    refs=[]
    for key in ("source_references","governed_references"):
        val=d.get(key) or []
        if isinstance(val,list): refs.extend([x for x in val if isinstance(x,dict)])
    if kind=="TRST":
        val=(d.get("evaluation") or {}).get("basis_references") or []
        refs.extend([{"identifier":x} for x in val if isinstance(x,str)])
    source_ids_ok=all(nonempty(x.get("identifier")) for x in refs)
    resolve("VAL-STR-008", source_ids_ok,
            "Serialized source/reference identifiers are preserved as explicit identifiers.",
            "One or more serialized source/reference identifiers are missing.")

    # ATT-local rules.
    if kind=="ATT":
        aid=d.get("attestation_identifier")
        resolve("VAL-ATT-001", isinstance(aid,str) and bool(ATT_RE.fullmatch(aid)),
                "Attestation has distinct canonical ATT identity.",
                "Attestation lacks distinct canonical ATT identity.","attestation_identifier")
        prov=d.get("provenance") or {}
        limitations=d.get("limitations",[])
        pml=prov.get("material_limitations",[])
        lim_ok=isinstance(limitations,list) and isinstance(pml,list)
        resolve("VAL-ATT-006", lim_ok,
                "Material limitations are explicitly representable in object/provenance fields.",
                "Material limitations are not preserved in controlled representation.","limitations")
        resolve("VAL-ATT-007", "trust_statement_identifier" not in d and "bounded_conclusion" not in d,
                "Attestation remains structurally distinct from Trust Statement.",
                "Attestation representation collapses into Trust Statement semantics.")

    # Minimum provenance context.
    p=d.get("provenance") or {}
    minprov=(p.get("mode") in PROV and nonempty(p.get("source_or_origin")) and
             (p.get("mode")!="derived" or nonempty(p.get("derivation_basis"))))
    resolve("VAL-PA-002", minprov,
            "Minimum provenance context is present.",
            "Minimum provenance context is incomplete.","provenance")

    # Authority context representation.
    auth=(d.get("attesting_authority") or {}) if kind=="ATT" else (d.get("attestor_attribution") or {})
    resolve("VAL-PA-006", auth.get("authority_context") in AUTH,
            "Controlled authority context is explicitly preserved.",
            "Authority context is missing or uncontrolled.")
    # Conflicts must be preservable where evaluation exists; ATT can preserve via limitations/notes.
    if kind=="TRST":
        ev=d.get("evaluation") or {}
        conflict_ok=isinstance(ev.get("material_conflicts"),list)
    else:
        conflict_ok=isinstance(d.get("limitations",[]),list)
    resolve("VAL-PA-009", conflict_ok,
            "Object provides an explicit governed location for material authority/conflict context.",
            "Authority/conflict context cannot be preserved in the serialized object.")

    # Relationship direction: container is source; every relationship has controlled type+target.
    rels=d.get("relationships") or []
    rel_ok=isinstance(rels,list) and all(isinstance(x,dict) and x.get("type") in REL and nonempty(x.get("target_identifier")) for x in rels)
    resolve("VAL-REL-002", rel_ok,
            "Relationship direction is explicit: containing object is source and target_identifier is target.",
            "Relationship direction is not deterministically represented.","relationships")

    # Lifecycle distinctions directly represented by current state.
    resolve("VAL-LV-004", d.get("lifecycle_state")=="draft" or d.get("lifecycle_state") in (LIFECYCLE-{"draft"}),
            "Draft and active are distinct controlled lifecycle values.",
            "Lifecycle value does not preserve draft/active distinction.","lifecycle_state")

    # Normative preservation checks that are representational, not substantive judgments.
    if kind=="ATT":
        evidence_ok=isinstance(d.get("evidence_references",[]),list)
        uncertainty_ok=isinstance(d.get("uncertainty",[]),list)
    else:
        ev=d.get("evaluation") or {}
        evidence_ok=isinstance(ev.get("basis_references"),list) and isinstance(ev.get("material_conflicts"),list) and isinstance(ev.get("material_exclusions"),list)
        uncertainty_ok=isinstance(d.get("uncertainty",[]),list)
    resolve("VAL-NRM-005", evidence_ok,
            "Evidence context has explicit governed representation.",
            "Evidence context representation is incomplete.")
    resolve("VAL-NRM-011", uncertainty_ok,
            "Uncertainty has explicit governed representation.",
            "Uncertainty representation is missing or uncontrolled.","uncertainty")

    governed_change_ok=(nonempty(d.get("version_identity")) and d.get("lifecycle_state") in LIFECYCLE)
    resolve("VAL-NRM-007", governed_change_ok,
            "Version and lifecycle fields provide explicit governed-change representation.",
            "Governed-change representation is incomplete.")
    hist_ok=(nonempty(d.get("version_identity")) and nonempty(d.get("created_at")) and nonempty(d.get("updated_at")))
    resolve("VAL-NRM-008", hist_ok,
            "Current object state carries version and temporal identity needed to distinguish governed states.",
            "Current/historical state representation is incomplete.")

    # Structural interoperability check only; substantive authority transfer remains Review.
    interop_ok=(nonempty((d.get("subject") or {}).get("identifier")) and
                (not rels or rel_ok) and auth.get("authority_context") in AUTH)
    resolve("VAL-NRM-012", interop_ok,
            "Object preserves identifier, relationship, and authority context needed for authority-preserving interoperability.",
            "Interoperability representation lacks identifier/relationship/authority context.")
    return results
...
def main():
    ...
    try:
        ...
        if "attestation_identifier" in d:
            ...
            results=object_local_results(results,"ATT",d)
            ...
            results=object_local_results(results,"TRST",d)
...
def main():
    ...
    try:
        ...
        report={"validation_report":{"report_version":"V1.0","target_identifier":ident,"target_profile":profile,
          "target_version":d.get("version_identity"),"requirements_set":"Satoshium Attestor Executable Validation Requirements",
          "requirements_version":"0.1","validator_identifier":"satoshium-attestor-validator","validator_version":"0.5",
          "run_at":datetime.now().astimezone().isoformat(),"aggregate_result":aggregate,"results":results,
          "summary":{"passed":sum(x["result"]=="pass" for x in results),
                     "failed":sum(x["result"]=="fail" for x in results),
                     "not_applicable":sum(x["result"]=="not-applicable" for x in results),
        ...
        report={"validation_report":{"report_version":"V1.0","target_identifier":None,"target_profile":None,
          "validator_identifier":"satoshium-attestor-validator","validator_version":"0.5",
          "run_at":datetime.now().astimezone().isoformat(),"aggregate_result":"error","results":[],
          "summary":{"passed":0,"failed":0,"not_applicable":0,"not_tested":0},"notes":[str(e)]}}
